...
{
    internal class QueryCustomerByName
    {
        public static async Task QueryCustomerByNameAsync(string graphqlUrl, string token, int CCC)
        {
            // Read supplierNo from user input (example: Airlift)
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var graphQLQuery = new
                {
                    ...
                    {{
                      useCompany(no: {CCC}) {{
                        associate(filter: {{ name: {{ _like: ""{customerName}%"" }}, customerNo: {{ _gt: 0 }} }}) 
                        {{
                          items {{
                            customerNo
                            companyNo
